...
void ProbabilityOracle::ComputeAll() {
  ...
  for (const auto& dice : all_states) {
    ...
    float is_yams = 0.0f, is_full = 0.0f, is_large = 0.0f, is_small = 0.0f;
    ...
    for (const auto& dice : all_states) {
        ...
        for (int i = 0; i < 32; ++i) {
            ...
            for(int k=0; k<4; ++k) {
                ...
                if (p > max_probs[k]) max_probs[k] = p;
            }
        }
...
std::vector<Action> ProYamsState::LegalActions() const {
  ...
  if (rolls_in_turn_ < 3) {
    ...
    if (can_reroll) {
      for (int i = 1; i < 32; ++i) moves.push_back(kActionRerollBase + i);
    }
  }
  return moves;
}
...
void ProYamsState::DoApplyAction(Action action_id) {
  if (is_chance_node_) {
    if (phase_ == 0) ResolveChanceCoefficients(action_id);
    else ResolveRoll(action_id);
    return;
  }
...
std::vector<std::pair<Action, double>> ProYamsState::ChanceOutcomes() const {
  static const std::vector<std::pair<Action, double>> kChanceOutcomes0 = []() {
    ...
    for (int i = 0; i < 720; ++i) outcomes.push_back({i, prob});
    return outcomes;
  }();

  if (phase_ == 0) return kChanceOutcomes0;
  ...
  static const std::vector<std::vector<std::pair<Action, double>>> kRollOutcomes = []() {
    ...
    for (int n = 1; n <= 5; ++n) {
      ...
      for (int i = 0; i < num_outcomes; ++i) all_outcomes[n].push_back({i, prob});
    }
    return all_outcomes;
  }();

  if (num_to_roll > 0 && num_to_roll <= 5) return kRollOutcomes[num_to_roll];

  std::vector<std::pair<Action, double>> outcomes;
  return outcomes; // Should not happen
}
...
int ProYamsState::CalculateScore(int col, int row, const std::vector<int>& dice) const {
    ...
    if (row <= kRow6) raw_score = (row + 1) * 5;
      ...
      if (row <= kRow6) { 
        ...
        raw_score = counts[target] * target;
          ...
          switch (row) {
            case kRowSS: case kRowLS: raw_score = sum; break;
            case kRowFH: {
              bool has3 = false, has2 = false;
              for (int c : counts) {
                if (c == 3) has3 = true; if (c == 2) has2 = true; if (c == 5) { has3 = true; has2 = true; } 
              }
              raw_score = (has3 && has2) ? (20 + sum) : 0;
              break;
            }
            case kRowK: {
              bool has4 = false; for (int c : counts) if (c >= 4) has4 = true;
            ...
            case kRowQ: { 
              ...
              if (small) raw_score = 45; else if (large) raw_score = 50; else raw_score = 0;
              break;
            }
            case kRow8: raw_score = (sum <= 8) ? (60 + 5 * (8 - sum)) : 0; break;
            case kRowY: {
              bool yams = false; int face = 0;
  ...
  if (!is_sec) {
      ...
      for (int other_p = 0; other_p < kNumPlayers; ++other_p) {
        if (board_[other_p][col][row] > max_existing) max_existing = board_[other_p][col][row];
      }
...
std::vector<double> ProYamsState::Returns() const {
  ...
  double p0_score = 0;
  for (int c = 0; c < kNumColumns; ++c) {
    bool p0_clean = false; bool p1_clean = false;
    ...
    int diff = raw0 - raw1;
    if (p0_clean) diff += 200; if (p1_clean) diff -= 200;
    p0_score += diff * column_coefficients_[c] * multiplier;
  }
...
std::string ProYamsState::ToString() const {
  std::stringstream ss;
  ss << "Phase: " << phase_ << "\n";
  ss << "Coeffs: "; for (int c : column_coefficients_) ss << c << " ";
  ss << "\nDice: "; for (int d : dice_) ss << d << " ";
...
std::string ProYamsState::ActionToString(Player player, Action action_id) const {
  if (player == kChancePlayerId) {
    std::stringstream ss;
    if (phase_ == 0) ss << "Chance (Coeffs): " << action_id;
    else ss << "Chance (Roll): " << action_id;
    return ss.str();
  }
  if (action_id >= kActionRerollBase) {
    int mask = action_id - kActionRerollBase;
    std::stringstream ss;
    ss << "Reroll [";
    for (int i = 0; i < kNumDice; ++i) if (CheckBit(mask, i)) ss << i << " ";
    ss << "]";
    return ss.str();
  }
...
std::string ProYamsState::InformationStateString(Player player) const { return ToString(); }
std::string ProYamsState::ObservationString(Player player) const { return ToString(); }
...
void ProYamsState::ObservationTensor(Player player, absl::Span<float> values) const {
  ...
  for (int p_offset = 0; p_offset < kNumPlayers; ++p_offset) {
    ...
    for (int c = 0; c < kNumColumns; ++c) {
      for (int r = 0; r < kNumRows; ++r) {
        if (board_[p][c][r] != -1) values[offset] = static_cast<float>(board_[p][c][r]) / 100.0f;
        else values[offset] = -1.0f; 
        offset++;
      }
    }
  }

  for (int i = 0; i < kNumDice; ++i) values[offset++] = static_cast<float>(dice_[i]) / 6.0f;

  if (!column_coefficients_.empty()) {
      for (int i = 0; i < kNumColumns; ++i) values[offset++] = static_cast<float>(column_coefficients_[i]) / 20.0f; 
  } else {
      offset += kNumColumns;
  }
  ...
  for (float p : probs) values[offset++] = p;
  ...
  for (int p_offset = 0; p_offset < kNumPlayers; ++p_offset) {
    ...
    for (int c = 0; c < kNumColumns; ++c) {
        int upper_sum = 0;
        int potential_add = 0;
        for (int r = 0; r <= kRow6; ++r) {
            int val = board_[p][c][r];
            if (val != -1) upper_sum += val;
            else potential_add += 5 * (r + 1);
        }
        float potential = (float)(upper_sum + potential_add);
        
        bool clean_dummy;
        int raw_score = CalculateColumnScore(c, p, clean_dummy);
        
        values[offset++] = (float)upper_sum / 105.0f;
        values[offset++] = potential / 105.0f;
        values[offset++] = (float)raw_score / 1000.0f; 
    }
  }
}
